 ...
 export function getCookies(key) {
     ...
     if (key != "reCreate") {
         ...
         for (let i = 0; i < arr.length; i++) {
             if (arr[i].indexOf("randomStr") != -1) {
                 recookie = arr[i].split("=")[1];
                 break;
             }
         }
     }
     ...
     if (recookie == "") {
         recookie = randomStr();
         document.cookie = "randomStr=" + recookie;
     }
     return recookie;
 }
 ...
 export const checkOs = {
     ...
     wechat() {
         return this.ua.match(/(MicroMessenger)\/([\d.]+)/i);
     },
     android() {
         return this.ua.match(/(Android);?[\s/]+([\d.]+)?/);
     },
     ios() {
         return this.ua.match(/(iPhone\sOS)\s([\d_]+)/);
     },
 };
...
export function emojiReplace(str, map) {
    for (var i = 1; i <= map.length; i++) {
        var img = '<img class="emoji-img" src="' + map[i - 1].image_path + '">';
        var res = map[i - 1].emoji;
        res = res.replace(/\[/, '\\[').replace(/\]/, '\\]').replace(/\//, '\\/').replace(/\(/, '\\(').replace(/\)/, '\\)').replace(/\'/, "\\'").replace(/\?/, "\\?").replace(/\|/, "\\|").replace(/\*/, "\\*").replace(/\$/, "\\$");        
        str = str.replace(new RegExp(res, "gm"), img)
    }
    return str;
}
